...
export const resetArtist = () => {
    return { type: types.RESET_ARTIST };
};
...
export const selectArtist = id => {
    return { type: types.SELECT_ARTIST, payload: id };
};

export const deselectArtist = id => {
    return { type: types.DESELECT_ARTIST, payload: id };
};

export const setRetired = ids => (dispatch, getState) =>
    SetRetiredProxy(ids.map(id => id.toString()))
    .then(() => dispatch({ type: types.RESET_SELECTION }))
...
export const setAgeRange = () => dispatch =>
    GetAgeRangeProxy()
    .then(result =>
        dispatch({ type: types.SET_AGE_RANGE, payload: result })
    );

export const setYearsActiveRange = () => dispatch =>
    GetYearsActiveRangeProxy()
    .then(result =>
        dispatch({ type: types.SET_YEARS_ACTIVE_RANGE, payload: result })
    );

export const searchArtists = (...criteria) => dispatch =>
    SearchArtistsProxy(...criteria)
    .then((result = []) =>
        dispatch({ type: types.SEARCH_ARTISTS, payload: result })
    );

export const findArtist = id => dispatch =>
    FindArtistProxy(id)
    .then(artist =>
        dispatch({ type: types.FIND_ARTIST, payload: artist })
    );
...
export const createArtist = props => dispatch =>
    ...
    .catch(error => {
        console.log(error);
        dispatch({ type: types.CREATE_ERROR, payload: error });
    });
