...
class ArtistIndex extends Component {
    ...
    renderList(artist) {
        ...
        return (
            <li className={classes} key={_id}>
                ...
                <div>
                    <span className="title"><strong>{artist.name}</strong></span>
                    <p>
                        <b>{artist.age}</b> years old
                        <br />
                        <b>{artist.yearsActive}</b> years active
                        <br />
                        <b>{artist.albums ? artist.albums.length : 0}</b> albums released
                    </p>
                </div>
    ...
    renderRetire() {
        if (this.props.selection.length) {
            return (
                <div>
                    <button className="btn" onClick={() => this.props.setRetired(this.props.selection)}>Retire</button>
                    <button className="btn" onClick={() => this.props.setNotRetired(this.props.selection)}>Unretire</button>
                </div>
            );
        }
    }
